...
![Histour 메인 화면](/assets/img/histour/home-ko.png){: width="75%" }
...
사용자는 지역/언어 설정 화면에서 원하는 언어를 선택할 수 있다.

![언어 설정 화면](/assets/img/histour/language-setting.png){: width="35%" }

사용자는 국가와 언어를 선택할 수 있으며, 선택한 언어에 따라 서비스 전체가 번역된다.

UI 문구는 한국어와 영어 데이터를 별도로 관리하는 방식을 사용하였다.
...
```
이 방식은 정적인 UI 문구를 효율적으로 관리할 수 있고, 번역 API를 매번 호출하지 않아도 된다는 장점이 있다.
...
반면 AI 역사 인물 응답은 사용자의 입력에 따라 매번 달라지기 때문에, 화면 UI처럼 문구를 하나하나 넣는 방식으로 처리할 수 없었다.

따라서 채팅 응답은 사용자가 선택한 언어 값을 AI 요청에 함께 전달하고, AI가 해당 언어로 답변하도록 설계했다.
...
![경복궁 상세 페이지](/assets/img/histour/place-detail.png){: width="65%" }
