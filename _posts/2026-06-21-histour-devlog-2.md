...
# 태조 이성계와 대화하는 AI 역사 체험은 어떻게 만들었을까?

> Histour 개발기 (2)

## MVP에서는 하나의 시나리오에 집중했다

Histour의 최종 목표는 다양한 역사 관광지와 인물을 제공하는 것이다.

하지만 MVP 단계에서는 여러 인물을 얕게 구현하기보다, **AI 역사 체험 기능 자체가 제대로 동작하는지 검증하는 것**이 더 중요하다고 판단했다.

그래서 경복궁과 태조 이성계를 중심으로 하나의 완성된 시나리오를 먼저 구현했다.
